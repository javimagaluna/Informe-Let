...
instalaciones_cambio %>%
  filter(instalaciones_cambio$Establecimiento=="hospitales_psiquiátricos" &instalaciones_cambio$Tasa>0.9| instalaciones_cambio$Establecimiento== "unidades_de_salud"& instalaciones_cambio$Tasa>0.9 |instalaciones_cambio$Establecimiento=="instalaciones_de_salud" & instalaciones_cambio$Tasa>5) %>%
  ggplot(., aes(x=pais, y=Tasa,fill= Establecimiento)) + theme_minimal()+ geom_bar(stat="identity", position=position_dodge())

# comparando --------------------------------------------------------------
instalaciones_SU[29,1]<-"Islandia"
instalaciones_SU[34,1]<-"Japón"


max_inst<- instalaciones_SU%>%filter(instalaciones_SU$hospitales_psiquiatricos>4 | instalaciones_SU$unidades_de_salud>4)


ggplot(instalaciones_SU, aes(hospitales_psiquiatricos, unidades_de_salud))+geom_point(alpha=0.5, col="purple4")+ theme_minimal()+ labs( y="Unidades de salud en hospitales generales", x= "Hospitales psiquiátricos", title="Relación entre unidades de salud mental de hospitales generales y hospitales psiquiátricos en 2016 a nivel mundial")+ ggrepel::geom_label_repel(aes(label = Pais), data = max_inst)

ggplot(instalaciones_SU, aes(hospitales_psiquiatricos, unidades_de_salud))+geom_point(alpha=0.5, col="purple4")+ theme_minimal()+ labs( y="Unidades de salud en hospitales generales", x= "Hospitales psiquiátricos", title="Relación entre unidades de salud mental de hospitales generales y hospitales psiquiátricos en 2016 a nivel mundial")+ ylim(c(0,1.2))+ xlim(c(0,1))+ geom_abline(intercept=0, slope=1)
